	...
	if (f_p_pcb != my_p_pcb) {
		...
		if(p_tcb == my_p_pcb->p_runtsk) {
			...
			if (t_p_pcb == my_p_pcb) {
				...
				/* 異なるプロセッサを指定 */
				/* レディーキューから外す */
                syslog_0(LOG_NOTICE, "make_non_runnable, task_manage, mig_tsk");
...
//changes
#define get_cyc_id(tskid) (INDEX_TSK(tskid) - (tnum_tsk - tnum_cyc))

ER
act_tsk(ID tskid)
{
    syslog_1(LOG_INFO, "activating task %d", tskid);
	TCB		*p_tcb;
	ER		ercd;
	bool_t	dspreq = false;
	PCB		*p_pcb;
    ...
    //changes
    if((cyc_id=get_cyc_id(tskid))>=0 && cyc_id<tnum_cyc) {
        syslog_2(LOG_INFO, "tskid = %d cyctim = %d", (int_t)tskid, (int_t)cycinib_table[cyc_id].cyctim);
        p_tcb_table[INDEX_TSK(tskid)]->current_deadline += cycinib_table[cyc_id].cyctim;
...
iact_tsk(ID tskid)
{
    syslog_1(LOG_INFO, "iactivating task %d", tskid);
	TCB		*p_tcb;
	ER		ercd;
	PCB		*p_pcb;
...
ext_tsk(void)
{
	...
	if ((p_runtsk->actprc == TPRC_NONE) || (p_runtsk->actprc == my_p_pcb->prcid)) {
        syslog_0(LOG_NOTICE,"make non_runnable, task_manage, ext_tsk");
...
ter_tsk(ID tskid)
{
	...
	/* 異なるプロセッサに割り付けられているタスクならエラーとする */
		...
		if (TSTAT_RUNNABLE(p_tcb->tstat)) {
			 ...
			 */
            syslog_0(LOG_INFO,"make non_runnable, task_manage, ter_tsk");
		...
		if (!(TSTAT_WAITING(p_tcb->tstat) && TSTAT_WAIT_WOBJ(p_tcb->tstat))) {
			...
			if ((p_tcb->actprc == TPRC_NONE) || (p_tcb->actprc == p_pcb->prcid)) {
				...
				if (TSTAT_RUNNABLE(p_tcb->tstat)) {
					 ...
					 * に発行可能であるので，ディスパッチ要求を出す必要はない． 
					 */
                    syslog_0(LOG_INFO,"make non_runnable, task_manage, ter_tsk 1");
					(void)make_non_runnable(p_tcb);
				}
				else if (TSTAT_WAITING(p_tcb->tstat)) {
					/* オブジェクト待ち以外の待ち状態の場合 */
				...
				if (TSTAT_RUNNABLE(p_tcb->tstat)) {
					 ...
					 * に発行可能であるので，ディスパッチ要求を出す必要はない． 
					 */
                    syslog_0(LOG_INFO,"make non_runnable, task_manage, ter_tsk 2");
					(void)make_non_runnable(p_tcb);
				}
				else if (TSTAT_WAITING(p_tcb->tstat)) {
					/* オブジェクト待ち以外の待ち状態の場合 */
