...
//global edf queue
LOCK    lck_g_edf_q;
QUEUE   g_edf_q[TNUM_TPRI];
int     g_edf_q_count;
uint16_t edf_primap;

bool_t init_pcb_done=false;
...
initialize_pcb(void)
{
    syslog_0(LOG_INFO, "queue edf init -3");
	uint_t	i;
	...
	//changes
	if (x_sense_mprc()) {
        syslog_0(LOG_INFO, "queue edf init -1");
        x_initialize_obj_lock(&lck_g_edf_q);
        syslog_0(LOG_INFO, "queue edf init");
        for(i = 0; i < TNUM_TPRI; i++) {
		    queue_initialize(&(g_edf_q[i]));
        }	
